...
def _convert_mgrs_to_lat_lon(mgrs_coord: str):
    m = mgrs.MGRS()
    return m.toLatLon(mgrs_coord)

def _convert_lat_lon_to_mgrs(lat_coord: float, lon_coord: float):
    m = mgrs.MGRS()
    c = m.toMGRS(lat_coord, lon_coord)
    return c

def convert_mgrs_lat_lon(mgrs_coord: str, lat_coord: str, lon_coord: str):
    if not mgrs_coord:
...
def _convert_ft_to_m(feet: float):
    return feet / 3.281

def _convert_m_to_ft(metres: float):
    return metres * 3.281

def convert_m_ft(metres: str, feet: str):
    if not metres:
        metres_output = _convert_ft_to_m(float(feet))
        feet_output = feet
    else:
        feet_output = _convert_m_to_ft(float(metres))
...
def _convert_epoch_to_date(epoch_time: int):
    try:
...
def _convert_date_to_epoch(date_time: str):
    try:
...
def convert_epoch_date(epoch_time: str, date_time: str):
    if not date_time:
        date_time_output = _convert_epoch_to_date(int(epoch_time))
...
def _convert_knots_to_mps(knots: float):
    return knots * 0.5144

def _convert_mps_to_knots(mps: float):
    return mps * 1.943844

def _convert_mps_to_kmh(mps: float):
    return mps / 1000 * 60 * 60

def _convert_kmh_to_mps(kmh: float):
    return kmh * 1000 / 60 / 60

def convert_knots_mps_kmh(knots: str, mps: str, kmh: str):
